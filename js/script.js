...
const getBooks = async function(books){
    ...
    genres.forEach(genre => {
        genre.addEventListener('click', function () {
            ...
            let booksClickedGenre = bookData.filter(book => book.genre.includes(genre.textContent))
            // console.log(booksClickedGenre)

            displayBooks(booksClickedGenre)
            if (genre === genres[0]) displayBooks(bookData);
        })
    })
    ...
    const clickBooks = bestRatingContainer.querySelectorAll(".item");
    clickBooks.forEach((book,i) => 
        book.addEventListener("click", function() {
            ...
            <img src="${bestRatingBooks[i].img}" alt="" class="book-info-img">
            <div class="book-info">
                <h2 class="book-info-title">${bestRatingBooks[i].title}</h2>
                <p class="book-info-item">Author: <span class="book-info-span">${bestRatingBooks[i].author}</span></p>
                <p class="book-info-item">Format: <span class="book-info-span">${bestRatingBooks[i].bookformat}</span></p>
                <p class="book-info-item">Genre: <span class="book-info-span">P${bestRatingBooks[i].genre}</span></p>
                <p class="book-info-item">Pages: <span class="book-info-span">${bestRatingBooks[i].pages}</span></p>
                <p class="book-info-item">Rating: <span class="book-info-span-rating">${bestRatingBooks[i].rating}</span></p>
            </div>
            </div>
            <div class="book-description">
                <p class="book-description-heading">Description</p>
                <p class="book-description-text">${bestRatingBooks[i].desc}</p>
            </div>`
